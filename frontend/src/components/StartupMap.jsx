...
const ENTITY_COLORS = {
  ...
  mentor: '#8B5CF6', corporate: '#0EA5E9', government_body: '#D97706',
}
const ENTITY_LABELS = {
  startup: '🚀 Startup', sme: '🏢 SME', college_ecell: '🎓 E-Cell',
  incubator: '🧪 Incubator', accelerator: '⚡ Accelerator', coworking: '💼 Coworking',
  investor: '💰 Investor', mentor: '🧠 Mentor', corporate: '🏛️ Corporate',
  government_body: '🏛️ Govt Body',
}
...
function PopupContent({ properties, onDetailClick }) {
  ...
  const addressParts = [p.city, p.district, p.state].filter(Boolean)
  const addressText = addressParts.join(', ')
  const mapsQuery = encodeURIComponent(`${p.name} ${addressText}`)
  const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${mapsQuery}`

  return (
    <div className="w-[300px] sm:w-80 bg-atlas-bg rounded-lg overflow-hidden shadow-2xl border border-atlas-border">
      {/* Top accent bar */}
      ...
      <div className="px-4 py-3 border-b border-atlas-border">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <h3 className="font-semibold text-atlas-text text-sm leading-tight">{p.name}</h3>
          </div>
          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full whitespace-nowrap flex-shrink-0 uppercase tracking-wide"
      ...
      <div className="px-4 py-2.5 space-y-2.5">
        ...
        {addressText && (
          <div className="flex items-start gap-1.5">
            <span className="text-atlas-muted mt-0.5">📍</span>
            <div className="min-w-0">
              <p className="text-xs text-atlas-muted leading-relaxed">{addressText}</p>
              <a
                href={mapsUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-[10px] text-brand-400 hover:text-brand-300 mt-0.5 transition-colors"
              >
                🗺️ Open in Google Maps <span className="opacity-60">↗</span>
              </a>
            </div>
          </div>
        )}
        ...
        {/* Description */}
        {p.description && (
          <p className="text-xs text-atlas-muted/80 leading-relaxed line-clamp-3">{p.description}</p>
        )}

        {/* Badges */}
        <div className="flex flex-wrap gap-1.5">
          {p.dpiit_recognized && (
            <span className="text-[10px] font-medium bg-emerald-500/15 text-emerald-300 px-2 py-0.5 rounded-full ring-1 ring-emerald-500/20">
              ✅ DPIIT Recognized
            </span>
          )}
          {p.unicorn_status === 'unicorn' && (
            <span className="text-[10px] font-bold bg-amber-500/15 text-amber-300 px-2 py-0.5 rounded-full ring-1 ring-amber-500/20">
              🦄 Unicorn
            </span>
          )}
          {p.is_women_led && (
            <span className="text-[10px] font-medium bg-pink-500/15 text-pink-300 px-2 py-0.5 rounded-full ring-1 ring-pink-500/20">
              👩 Women-led
            </span>
          )}
          {p.nsa_winner && (
            <span className="text-[10px] font-medium bg-rose-500/15 text-rose-300 px-2 py-0.5 rounded-full ring-1 ring-rose-500/20">
              🏆 NSA Winner
            </span>
          )}
        </div>
        ...
        {(Array.isArray(sectors) ? sectors : []).length > 0 && (
          <div className="flex flex-wrap gap-1">
            {(Array.isArray(sectors) ? sectors : []).slice(0, 5).map(s => (
              <span key={s} className="text-[10px] bg-atlas-surface text-atlas-muted px-2 py-0.5 rounded-full border border-atlas-border/60">{s}</span>
            ))}
          </div>
        )}
        ...
        <div className="flex items-center gap-3 text-[10px] text-atlas-muted/70">
          {p.founded_year && <span className="flex items-center gap-1">📅 {p.founded_year}</span>}
          {p.linkedin_team_size && <span className="flex items-center gap-1">👥 {p.linkedin_team_size}</span>}
          {p.funding_display && p.funding_display !== 'Bootstrapped' && (
            <span className="flex items-center gap-1">💰 {p.funding_display}</span>
          )}
          {p.website && isValidHttpUrl(p.website) && (
            <a
            ...
            >
              🌐 Website <span className="opacity-50">↗</span>
            </a>
          )}
        </div>
        ...
        <div className="pt-1 flex flex-wrap gap-1.5">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1 text-[10px] px-2 py-1 rounded-md bg-atlas-surface text-atlas-muted hover:text-atlas-text transition-colors"
          >
            🔗 All Profiles
          </a>
        </div>
      ...
      {pointFeatures && (showClusters || showPoints || showHeatmap) && (
        ...
        >
          ...
          <Layer id="entity-points" type="circle"
            ...
            paint={{
              'circle-color': ['match', ['get', 'entity_type'], 'startup', '#3B82F6', 'sme', '#10B981', 'college_ecell', '#FBBF24', 'incubator', '#A855F7', 'accelerator', '#EC4899', 'coworking', '#14B8A6', 'investor', '#EF4444', 'mentor', '#8B5CF6', 'corporate', '#0EA5E9', 'government_body', '#D97706', '#64748B'],
          ...
          <Layer id="entity-icons" type="symbol"
            ...
            layout={{
              'text-field': ['match', ['get', 'entity_type'], 'startup', '🚀', 'sme', '🏢', 'college_ecell', '🎓', 'incubator', '🧪', 'accelerator', '⚡', 'coworking', '💼', 'investor', '💰', 'mentor', '🧠', 'corporate', '🏛️', 'government_body', '🏛️', '📍'],
