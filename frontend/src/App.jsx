...
import { useLiveEntities } from './hooks/useSupabase'
...
export default function App() {
  ...
  // v4.10.00 — Supabase Realtime
  const { liveEntities, toasts, clearToast, connected } = useLiveEntities(true)
  ...
  useEffect(() => {
    fetch('/api/entities/analytics/overview')
      .then(r => r.json())
      .then(data => {
        // v4.10.10 — Handle new {mapped, nationwide} structure
        setOverview(data)
      })
      .catch(console.error)
  }, [])
  ...
  return (
    <ErrorBoundary>
      <div className="relative w-full h-screen overflow-hidden bg-atlas-bg">
        ...
        {/* v4.10.00 — Real-Time Toast Notifications */}
        {toasts.length > 0 && (
          <div className="fixed top-16 right-3 z-50 flex flex-col gap-2 pointer-events-none">
            {toasts.map(toast => (
              <div
                ...
                className="pointer-events-auto glass rounded-lg px-3 py-2 shadow-lg max-w-xs cursor-pointer animate-slide-in-right hover:bg-atlas-surface transition-all"
              >
                <div className="flex items-start gap-2">
                  <span className="text-lg flex-shrink-0">{toast.emoji}</span>
                  <div className="min-w-0">
                    <p className="text-xs text-atlas-text font-medium truncate">{toast.name}</p>
                    <p className="text-[10px] text-atlas-muted">{toast.city}, {toast.state}</p>
                  </div>
                  <span className="text-[10px] text-atlas-muted/50 flex-shrink-0">{toast.timestamp}</span>
                </div>
              </div>
            ))}
            {connected && (
              <div className="flex items-center gap-1.5 px-2 py-1">
                ...
                <span className="text-[10px] text-atlas-muted/70">Live</span>
              </div>
            )}
          </div>
        )}
