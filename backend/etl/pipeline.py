...
class ETLPipeline:
    ...
    async def run_live_startup_india(self, roles=None, max_pages=2) -> Dict:
        """
        Run live extraction from Startup India portal.
        
        Args:
            roles: List of roles to extract (e.g., ['mentor', 'investor'])
            max_pages: Number of pages to scrape per role
        
        Returns:
            Pipeline report with extraction stats.
        """
        from .live_extract import StartupIndiaLiveExtractor
        ...
        logger.info(f"═══ Live ETL Run {run_id} (Startup India) ═══")
        
        extractor = StartupIndiaLiveExtractor(delay_seconds=1.5)
        try:
            t0 = time.time()
            logger.info("Phase 1/3: EXTRACT — Fetching from Startup India...")
            
            raw_records = await extractor.extract_all(roles=roles, max_pages=max_pages)
            
            total_records = sum(len(v) for v in raw_records.values())
            report["extract"] = {
                "records": total_records,
                "by_role": {k: len(v) for k, v in raw_records.items()},
                "duration_sec": round(time.time() - t0, 2),
            }
            logger.info(f"  ✓ Extracted {total_records} records from {len(raw_records)} roles")
            
            # Flatten records
            all_records = []
            for role_records in raw_records.values():
                all_records.extend(role_records)
            
            # Transform
            t1 = time.time()
            logger.info("Phase 2/3: TRANSFORM — Normalizing records...")
            transformed = self.transformer.transform_all(all_records)
            report["transform"] = {
            ...
            # Load
            t2 = time.time()
            logger.info("Phase 3/3: LOAD — Writing to database...")
            load_stats = self.loader.load_all(transformed)
            report["load"] = {
                **load_stats,
                "duration_sec": round(time.time() - t2, 2),
            }
            logger.info(f"  ✓ Loaded: {load_stats}")
            ...
            report["status"] = "failed"
            report["errors"].append(str(e))
            logger.error(f"Live ETL failed: {e}")
        finally:
            await extractor.close()
        
        self._run_history.append(report)
        logger.info(f"═══ Live ETL {report['status'].upper()} ({report.get('total_duration_sec', 0)}s) ═══")
        return report
