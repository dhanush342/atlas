...
# ─── Endpoints ────────────────────────────────────────────────────────────────
# ─── v4.10.00 — SSE Streaming Chat (Token-by-Token) ──────────────────────────
...
async def _stream_tokens(messages: List[dict], web_results: Optional[List[dict]] = None):
    """
    Async generator that yields SSE events token-by-token.
    Uses TextIteratorStreamer for non-blocking token generation.
    """
    pipeline = _get_chat_pipeline()
    
    # Yield initial "thinking" event
    yield f"data: {_json.dumps({'type': 'status', 'text': 'Thinking...'})}\n\n"
    await asyncio.sleep(0.01)
    
    if not pipeline:
        # Fallback mode
        if web_results:
            search_msg = _json.dumps({'type': 'token', 'text': '🌐 Search results:\n\n'})
            yield f"data: {search_msg}\n\n"
            for r in web_results[:5]:
                item_msg = _json.dumps({'type': 'token', 'text': f'• {r["title"]}: {r["snippet"][:150]}...\n'})
                yield f"data: {item_msg}\n\n"
                await asyncio.sleep(0.05)
        else:
            yield f"data: {_json.dumps({'type': 'token', 'text': '🤖 Running in lightweight mode. Ask about unicorns, fintech, SaaS, or accelerators!'})}\n\n"
    ...
    try:
        ...
        streamer = TextIteratorStreamer(pipeline.tokenizer, skip_prompt=True, skip_special_tokens=True)
        
        # Run generation in background thread
        generation_kwargs = dict(
            inputs.to(pipeline.model.device),
            streamer=streamer,
            max_new_tokens=512,
            do_sample=True,
            temperature=0.7,
        ...
        thread = Thread(target=pipeline.model.generate, kwargs=generation_kwargs)
        thread.start()
        
        # Stream tokens as they arrive
        for token_text in streamer:
            if token_text.strip():
                yield f"data: {_json.dumps({'type': 'token', 'text': token_text})}\n\n"
                await asyncio.sleep(0.01)
...
async def chat_stream(request: Request, body: ChatRequest):
    """
    SSE streaming chat endpoint. Yields tokens token-by-token.
    
    Content-Type: text/event-stream
    
    Example event:
        data: {"type": "token", "text": "India's startup..."}
    ...
    if not ok:
        audit_log("chat_stream_rejected", req_id, details={"reason": err}, severity="warning")
        raise HTTPException(status_code=400, detail=err)
    
    injection_score = detect_prompt_injection(user_text)
    if injection_score > 0.3:
        audit_log("stream_injection_suspicious", req_id, details={"score": injection_score}, severity="warning")
    ...
    audit_log("chat_stream_start", req_id, details={"web_search": bool(web_results)}, severity="info")
    
    return StreamingResponse(
        _stream_tokens(messages, web_results),
