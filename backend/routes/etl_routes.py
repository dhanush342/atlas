    ...
    return {
        "sources": [
            ...
            },
            {
                ...
                "provides": ["mentors", "investors", "incubators", "accelerators", "corporates", "government bodies"],
                "update_frequency": "weekly",
                "status": "live_etl_ready",
            },
        ],
        "pipeline": {
...
async def trigger_live_etl(
    request: Request,
    roles: str = Query("mentor,investor,incubator", description="Comma-separated roles to extract"),
    max_pages: int = Query(2, ge=1, le=10),
):
    """Trigger live ETL from Startup India portal."""
    req_id = getattr(request.state, "request_id", "unknown")
    from ..security import audit_log

    role_list = roles.split(",")
    pipeline = _get_pipeline()
    report = await pipeline.run_live_startup_india(roles=role_list, max_pages=max_pages)

    audit_log("live_etl", req_id, details={"roles": role_list, "pages": max_pages, "status": report["status"]}, severity="info")
    return {"message": "Live ETL completed", "report": report}
