...
from ..utils import row_to_dict, format_funding, sanitize_like, rank_search_results
...
from ..analytics import get_ecosystem_breakdown, get_top_states, get_state_detail
from ..matching import match_mentors_for_startup, match_startups_for_investor
from ..cache import entity_cache, facet_cache
...
async def get_clusters(
    request: Request,
    min_lng: float = Query(68.0, ge=50, le=110),
    max_lng: float = Query(97.0, ge=50, le=110),
    min_lat: float = Query(6.0, ge=-10, le=50),
    max_lat: float = Query(37.0, ge=-10, le=50),
...
async def get_viewport_summary(
    request: Request,
    min_lng: float = Query(..., ge=50, le=110),
    max_lng: float = Query(..., ge=50, le=110),
    min_lat: float = Query(..., ge=-10, le=50),
    max_lat: float = Query(..., ge=-10, le=50),
    entity_type: Optional[str] = Query(None),
):
...
async def search_entities(
    ...
    raw_results = [row_to_dict(r) for r in rows]
    ranked_results = rank_search_results(raw_results, q)

    audit_log("search", req_id,
              details={"q": q[:50], "total": total, "limit": limit, "offset": offset},
              severity="info")

    return {"results": ranked_results, "total": total, "limit": limit, "offset": offset}
...
async def analytics_overview(request: Request):
    ...
    return {
        "mapped": stats,
        "nationwide": {
            "dpiit_startups": 212283,
            "women_led_startups": 102054,
            "unicorns": 131,
            "women_led_unicorns_pct": 18,
            "mentors_trained": 677,
            "nsa_categories": 20,
            "source": "PIB Press Release 2026, DPIIT, Startup India",
            "source_url": "https://www.pib.gov.in/PressReleasePage.aspx?PRID=2197662",
        },
    }
...
# ─── v4.00.02 — Analytics Endpoints ──────────────────────────────────────────

@router.get("/analytics/ecosystem")
async def analytics_ecosystem(
    request: Request,
    entity_type: Optional[str] = Query(None),
    min_startups: int = Query(0, ge=0),
):
    """Full ecosystem breakdown by state and city."""
    types = entity_type.split(",") if entity_type else None
    result = get_ecosystem_breakdown(entity_types=types, min_startups=min_startups)
    return result


@router.get("/analytics/top-states")
async def analytics_top_states(
    request: Request,
    limit: int = Query(10, ge=1, le=50),
):
    """Quick top states summary by ecosystem strength."""
    return {"states": get_top_states(limit=limit)}


@router.get("/analytics/state/{state}")
async def analytics_state_detail(request: Request, state: str):
    """Detailed analytics for a single state."""
    return get_state_detail(state)


# ─── v4.00.02 — Mentor Matching Endpoints ───────────────────────────────────

@router.get("/match/mentors")
async def match_mentors(
    request: Request,
    startup_slug: Optional[str] = Query(None),
    state: Optional[str] = Query(None),
    sectors: Optional[str] = Query(None),
    limit: int = Query(10, ge=1, le=50),
    prefer_women_led: bool = Query(False),
):
    """
    Match mentors to a startup or discover mentors by criteria.
    
    Usage:
        /api/entities/match/mentors?startup_slug=flipkart&limit=8
        /api/entities/match/mentors?state=Karnataka&sectors=AI,fintech&limit=15
    """
    sector_list = sectors.split(",") if sectors else None
    
    result = match_mentors_for_startup(
        startup_slug=startup_slug or "",
        limit=limit,
        discovery_mode=(startup_slug is None),
        state=state,
        sectors=sector_list,
...
@router.get("/match/startups")
async def match_startups_for_investor_endpoint(
    request: Request,
    investor_slug: str = Query(...),
    state: Optional[str] = Query(None),
    sectors: Optional[str] = Query(None),
    stage: Optional[str] = Query(None),
    min_funding: float = Query(0, ge=0),
    limit: int = Query(10, ge=1, le=50),
):
    """Match startups to an investor based on criteria."""
    sector_list = sectors.split(",") if sectors else None
    
    result = match_startups_for_investor(
